    ...
    for i in tqdm(range(len(dataset))):
        ...
        for sampled_step_id in range(len(example['actions'])):
            ...
            if sampled_step_id > 0:    
                action_history = [example['actions'][step_id]['step_action_nl'] for step_id in range(sampled_step_id)]
                action_history = '\n'.join(action_history)

            if args.use_new_format:
                if example['actions'][sampled_step_id]['acc_tree_visible_before']:
                    acc_tree = example['actions'][sampled_step_id]['acc_tree_visible_before']

                    if 'acc_tree_other_before' in example['actions'][sampled_step_id] and example['actions'][sampled_step_id]['acc_tree_other_before'] and len(example['actions'][sampled_step_id]['acc_tree_other_before']) > 0:
                        acc_tree = acc_tree + example['actions'][sampled_step_id]['acc_tree_other_before']
                else:
            ...
            if args.use_new_format:
                image_path = os.path.join(root, example['folder'], f'screenshot_som_crop_{sampled_step_id}.png')
            else:
                image_path = os.path.join(root, example['folder'], f'screenshot_som_{sampled_step_id}.png')

            if not os.path.exists(image_path):
                image_path = os.path.join(example['folder'], f'screenshot_som_{sampled_step_id}.png')

            sample = {'system_message':system_message, 'prompt_message': prompt_message, 'image_path': image_path, 'actions': example['actions'][sampled_step_id]}
...
class WebTrajDataOrderedCollator:
    ...
    def __call__(self, data):
        ...
        batch = {'input_ids': input_ids, 'labels': labels, 'pixel_values': pixel_values, 'image_sizes': image_sizes}
