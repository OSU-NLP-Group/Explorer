...
class Phi3Generator:
    def __init__(self, model, processor, add_repetition_penalty=False, repetition_penalty=1.1, max_new_tokens=512, temperature=0.7, do_sample=True):
        self.model = model
...
class Phi3MiniGenerator:
    ...
    def get_response(self, messages, image=None):
        ...
        generate_ids = self.model.generate(**inputs, eos_token_id=self.processor.eos_token_id, **generation_args) 
...
class Mistral7BInstructGenerator:
    def __init__(self, model, tokenizer, add_repetition_penalty=False, repetition_penalty=1.1, max_new_tokens=500, temperature=0.7, do_sample=True):
        self.model = model
    ...
    async def request(self, messages: list = None, image = None, max_tokens: int = 500, temperature: float = 0.7) -> (str, str):
        try:
            cpu_count = multiprocessing.cpu_count()
            with ThreadPoolExecutor(max_workers=cpu_count * 2) as pool:
                future_answer = pool.submit(self.chat, messages, image)
...
class LlavaNextGenerator:
    def __init__(self, model, processor, max_new_tokens=512, temperature=0.7, do_sample=True):
        self.model = model
        self.processor = processor
        self.max_new_tokens = max_new_tokens
        self.temperature = temperature
    ...
    def get_response(self, messages, image=None):
        ...
        messages_new = []
        messages_new.append({"role": "assistant", "content": [{"type": "text", "text": messages[0]['content']}]})
        messages_new.append({"role": "user", "content": [{"type": "text", "text": messages[-1]['content']}, {"type": "image"}]})
...
class Qwen2VLGenerator:
    ...
    def get_response(self, messages, image=None):
        ...
        messages_new = []
        messages_new.append({"role": "system", "content": [{"type": "text", "text": system_prompt}]})

        if '<|image_1|>' in user_prompt:
            messages_new.append({"role": "user", "content": [{"type": "text", "text": user_prompt_0}, {"type": "image"}, {"type": "text", "text": user_prompt_1}]})
        else:
            messages_new.append({"role": "user", "content": [{"type": "text", "text": user_prompt_1}]})
        ...
        if image is not None:
            ...
            inputs = self.processor(text=prompt, images=[image], padding=True, return_tensors="pt").to("cuda:0")
        else:
            # print('image is None')
            inputs = self.processor(text=prompt, padding=True, return_tensors="pt").to("cuda:0")
        ...
        # remove input tokens 
        generate_ids = generate_ids[:, inputs['input_ids'].shape[1]:]
        response = self.processor.batch_decode(generate_ids, skip_special_tokens=True, clean_up_tokenization_spaces=True)[0]
