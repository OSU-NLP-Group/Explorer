...
from evals.mind2web_live_eval.agent.Environment.html_env.async_env import ActionExecutionError
import re
import toml
import json
import traceback
...
from evals.mind2web_live_eval.agent.Environment.html_env.actions_sync import create_id_based_action, create_none_action
...
def step_evaluate(page: Page, evaluate_steps=[], input_path=None, element_value=None, args=None):
    """Evaluate step score"""
    step_score = 0
    match_result = []
        ...
        if evaluate["score"] != 1:
            ...
            if match_function == "url_exactly_match":
                ...
                score = ElementEvaluator.path_exact_match(
                    input_path, evaluate["reference_answer"], method, page.content(), input_netloc,
                    evaluate["netloc"])
...
def run_task(
    ...
    for i in range(3):
        try:
            observation = env.get_obs(image_path=None, som_model=som_model, caption_model_processor=caption_model_processor)
            break
        except:
            logger.info('Error in get_obs() try {}: {}'.format(num_tries, traceback.format_exc()))
            num_tries += 1
            time.sleep(5)
    ...
    if args.output_screenshots:
        os.makedirs(os.path.join(config['files']['out_file_path'], str(task_index)), exist_ok=True)
        observation['image_obs'].save(os.path.join(config['files']['out_file_path'], str(task_index), "som_screenshot_{}.png".format(num_global_steps)))
    ...
    while num_steps < max_steps + additional_steps and num_global_steps < args.num_global_steps:
        error_message = ""
        total_step_score = 0
        step_reward = {}
        status_description = ""
        ...
        try:
            ...
            if out_put:
                ...
                try:
                    cur_action = create_id_based_action(action_grounded, args.use_google_api)
                except:
                ...
                current_trace = {"thought": '',
                     "action": pred['action_natural_language'], "reflection": ''}
                ...
                if task_mode == "batch_tasks":
                    evaluate_steps, match_result = step_evaluate(page=env.page, evaluate_steps=evaluate_steps,
                                                                    input_path=selector, element_value=element_value, args=args)
                    for evaluate in evaluate_steps:
                ...
                try:
                    ...
                    observation = env.get_obs(image_path=image_path, som_model=som_model, caption_model_processor=caption_model_processor)
                    logger.info(f"Acc tree: {observation['content_str']}")
                except:
                ...
                if args.output_screenshots:
                    os.makedirs(os.path.join(config['files']['out_file_path'], str(task_index)), exist_ok=True)
                    observation['image_obs'].save(os.path.join(config['files']['out_file_path'], str(task_index), "som_screenshot_{}.png".format(num_global_steps+1)))
