from evals.mind2web_live_eval.agent.Environment.html_env.async_env import AsyncHTMLEnvironment
from evals.mind2web_live_eval.agent.Environment.html_env.browser_env import ScriptBrowserEnv
...
if __name__ == "__main__":
    ...
    parser.add_argument("--planning_text_model", type=str, choices=["gpt-3.5-turbo", "gpt4o_2", "gpt-4o", "gpt-4o-mini", "phi-3v", "phi-3.5v", 'phi3mini', 'Mistral-7B-Instruct', 'llava-v1.6-mistral-7b', 'llava-v1.6-vicuna-13b-hf', 'qwen2-vl-7b', 'qwen2-vl-72b', 'intern-vl-8b', 'phi3mini'], default="gpt-3.5-turbo")
    ...
    parser.add_argument('--omit-verbose-logging', action='store_true', help='Omit verbose logging in planning module')
    parser.add_argument('--n-tries', type=int, default=3, help='no. of tries if not in json format')
    parser.add_argument('--ckpt-path', type=str, default='', help='path to Phi-3V ckpt')
    parser.add_argument('--add-repetition-penalty', action='store_true', help='add rep penalty for SLM')
    parser.add_argument('--repetition-penalty', type=float, default=1.1, help='rep penalty for SLM')
    parser.add_argument('--temp', type=float, help='decoding temperature', required=True)

    parser.add_argument('--add-first-action', action='store_true', help='auto add the first goto url action')
    parser.add_argument('--skip-image-step0', action='store_true', help='skip (blank) image at step 0 of VLM, modify prompt to skip reference of image at step 0')
    parser.add_argument('--omit-tab-name', action='store_true', help='')

    parser.add_argument('--num-global-steps', type=int, default=25, help='no. of global steps')
    parser.add_argument('--press-enter-after-fill', action='store_true', help='press enter after filling the form')
    parser.add_argument('--use-visible-acc-tree', action='store_true', help='use visible acc tree')
    parser.add_argument('--debug', action='store_true', help='just run a single task')
    parser.add_argument('--output-screenshots', action='store_true', help='', default=True)

    parser.add_argument('--omit-image', action='store_true', help='use only textual input')
    parser.add_argument('--use-complete-acc-tree', action='store_true', help='use complete acc tree for observation')
    parser.add_argument('--use-greedy', action='store_true', help='use greedy decoding')
    parser.add_argument('--num-crops', type=int, default=16, help='no. of crops for Phi-3.5V model')
    parser.add_argument('--max-len-acctree', type=int, default=None, help='max length for acc tree')
    parser.add_argument('--screenshot-height', type=int, default=-1, help='height of screenshot to crop to if > -1')
    parser.add_argument('--process-sleep', type=int, default=5, help='sleep time in process_new')
    parser.add_argument('--use-google-api', action='store_true', help='use google api directly for google search')

    # Define argument for a list of numbers with the --numbers flag
    parser.add_argument("--task-list", type=int, nargs="+", default=[], help="list of tasks to evaluate on")
