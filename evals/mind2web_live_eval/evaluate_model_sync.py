...
# evaluate tools
from evals.mind2web_live_eval.evaluate.evaluate_utils import read_config, read_file, read_json_file
...
def run_experiment(args, task_range, experiment_config):
    ...
    for task_index in tqdm(task_range):
        ...
        env = ScriptBrowserEnv(args,  browser_type='chrome', viewport_size={'width': args.viewport_width, 'height': args.viewport_height})
...
def main_sync(args,
    ...
    if config['basic']['task_mode'] == "batch_tasks":
        ...
        task_range = get_task_range(args, config['basic']['task_mode'], config['files']['batch_tasks_file_path'], -1)
